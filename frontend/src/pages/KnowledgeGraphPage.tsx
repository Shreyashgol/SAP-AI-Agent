    ...
    <div className="flex h-full">
      ...
      <div className="w-80 border-r bg-white flex flex-col">
        ...
        <div className="p-4 border-b">
          ...
          {buildJobId && (
            <p className="text-xs text-gray-500 mt-1.5">Job: {buildJobId}</p>
          )}
        </div>
        ...
        <div className="p-4 border-t mt-auto">
          ...
          {Object.entries(DOMAIN_COLORS).map(([domain, color]) => (
            <div key={domain} className="flex items-center gap-2 mb-1.5">
              ...
              />
              <span className="text-xs text-gray-700 capitalize">{domain}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="w-80 border-l bg-white overflow-y-auto">
      <div className="p-4 border-b flex items-center justify-between">
        <div>
          <p className="font-semibold text-gray-900">{node.node_label}</p>
          <p className="text-xs text-gray-500 capitalize">{node.domain}</p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-4">
        ...
        {edges.length === 0 ? (
          <p className="text-sm text-gray-400">No edges.</p>
        ) : (
...
function EdgeCard({
  ...
  return (
    <div className="border rounded-lg p-3 text-sm">
      ...
      {edge.is_admin_confirmed && (
        <span className="inline-block mt-1 text-xs text-green-600">✓ Confirmed</span>
      )}
    </div>
  );
}
          ...
          <tbody>
            {edges.slice(0, 20).map((e) => (
              <tr key={e.id} className="border-t border-yellow-200">
                <td className="py-1 pr-4 font-medium text-gray-700">{e.relation_name}</td>
                <td className="py-1 pr-4 text-gray-500">{e.edge_type}</td>
                <td className="py-1 pr-4">
                  <span className={`px-1.5 py-0.5 rounded-full ${CONFIDENCE_CLASS(e.confidence)}`}>
                    {(e.confidence * 100).toFixed(0)}%
                  </span>
                </td>
                <td className="py-1 font-mono text-gray-500 truncate max-w-xs">{e.join_condition}</td>
              </tr>
            ))}
          </tbody>
        </table>
