...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border font-medium ${styles[severity] ?? "bg-gray-100 text-gray-600"}`}>
      {icons[severity]}
      {severity}
    </span>
  );
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded font-medium ${styles[status] ?? "bg-gray-100 text-gray-500"}`}>
      {icons[status]}
      {status}
    </span>
  );
...
function CreateRuleForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 border border-gray-200 rounded-xl bg-gray-50">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-gray-900">New Alert Rule</h3>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Check frequency</label>
          <select
...
function CreateReportForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 border border-gray-200 rounded-xl bg-gray-50">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-gray-900">New Report Schedule</h3>
        <button type="button" onClick={onClose} className="text-gray-400 hover:text-gray-600">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-700 mb-1">
          Cron expression
          <span className="ml-2 font-normal text-gray-400">(minute hour dom month dow)</span>
        </label>
        <input
          value={cron}
          ...
          placeholder="0 8 * * 1"
        />
        <p className="text-xs text-gray-400 mt-1">e.g. "0 8 * * 1" = every Monday at 8am</p>
      </div>
        ...
        <div className="space-y-2">
          {questions.map((q, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="flex-1 text-sm text-gray-700 bg-white border border-gray-200 rounded px-2 py-1 truncate">{q}</span>
              <button
                type="button"
...
export default function AlertsPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-gray-900">Alerts & Reports</h1>
      ...
      {tab === "active" && (
        <div className="space-y-3">
          {(alerts ?? []).length === 0 ? (
            ...
            (alerts ?? []).map((alert) => (
              <div key={alert.id} className="border border-gray-200 rounded-xl p-4 bg-white space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    {alert.rca_summary && (
                      <p className="text-xs text-gray-500">{alert.rca_summary}</p>
                    )}
                  </div>
                  {alert.status === "active" && (
                ...
                {(alert.suggested_questions ?? []).length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    <span className="text-xs text-gray-400">Investigate:</span>
                    {(alert.suggested_questions as string[]).map((q, i) => (
                      <span key={i} className="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full border border-blue-100">{q}</span>
                    ))}
                  </div>
                )}
        ...
        <div className="space-y-3">
          {(rules ?? []).length === 0 && !showRuleForm && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-400">No alert rules. Create one to start monitoring.</p>
            </div>
          )}
          {(rules ?? []).map((rule) => (
            <div key={rule.id} className="border border-gray-200 rounded-xl p-4 bg-white flex items-center gap-4">
              <div className="flex-1 space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-sm text-gray-900">{rule.name}</span>
                  <SeverityBadge severity={rule.severity} />
                  {!rule.is_active && (
                    <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-400 rounded">Paused</span>
                  )}
                </div>
      ...
      {tab === "reports" && (
        <div className="space-y-3">
          {(schedules ?? []).length === 0 && !showReportForm && (
            <div className="text-center py-16">
              <Bell className="w-12 h-12 text-gray-200 mx-auto mb-3" />
              <p className="text-gray-400">No report schedules. Create one to automate recurring reports.</p>
            </div>
          )}
          {(schedules ?? []).map((sched) => (
            <div key={sched.id} className="border border-gray-200 rounded-xl p-4 bg-white space-y-2">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm text-gray-900">{sched.name}</span>
                    {!sched.is_active && (
                      <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-400 rounded">Paused</span>
                    )}
                  </div>
                  <p className="text-xs font-mono text-gray-500">{sched.cron_expression}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => triggerReport.mutate(sched.id)}
                    disabled={triggerReport.isPending}
                    className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 disabled:opacity-40"
                    title="Run now"
                  >
              ...
              <div className="flex flex-wrap gap-1.5">
                {sched.questions.map((q, i) => (
                  <span key={i} className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full truncate max-w-xs">{q}</span>
                ))}
              </div>
            </div>
          ))}
