...
function DomainPill({ domain }: { domain: string }) {
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${DOMAIN_COLORS[domain] ?? "bg-gray-100 text-gray-600"}`}>
      {domain}
    </span>
  );
}
    ...
    <div className="fixed inset-0 bg-black/40 z-40 flex justify-end">
      <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-xl flex flex-col">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="font-semibold text-gray-900">Edit Entity</h2>
        ...
        <div className="p-4 space-y-4 flex-1">
          ...
          <div className="pt-2 border-t text-xs text-gray-400 space-y-1">
            ...
            {entity.is_human_override && (
              <p className="text-blue-600 font-medium">Human override — AI won't overwrite this.</p>
            )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-400 ml-auto">{total} entities</span>
      </div>
            ...
            <tbody>
              {entities.map((e) => (
                <tr key={e.id} className="border-b hover:bg-gray-50 transition-colors">
                  <td className="py-2.5 px-4 font-medium text-gray-900 text-sm">{e.entity_name}</td>
                  <td className="py-2.5 px-4">
                    <DomainPill domain={e.domain} />
                  </td>
                  <td className="py-2.5 px-4 text-xs text-gray-400 font-mono">{e.pack_source}</td>
                  <td className="py-2.5 px-4">
                    <ConfidenceBadge value={e.confidence} />
                  </td>
                  <td className="py-2.5 px-4">
                    {e.is_human_override ? (
                      <span className="text-xs text-blue-600 font-medium">Human override</span>
                    ) : e.is_ai_generated ? (
                      <span className="text-xs text-yellow-600">AI — pending review</span>
                    ) : (
                      <span className="text-xs text-green-600">Pack</span>
                    )}
                  </td>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="ml-auto px-3 py-1.5 bg-gray-100 text-gray-700 rounded-md text-sm
                     hover:bg-gray-200 disabled:opacity-50"
        >
          {seedKPIs.isPending ? "Seeding…" : "Seed System KPIs"}
        </button>

        <span className="text-sm text-gray-400">{data?.total ?? 0} KPIs</span>
      </div>
            ...
            <tbody>
              {kpis.map((k) => (
                <tr key={k.id} className="border-b hover:bg-gray-50">
                  <td className="py-2.5 px-4">
                    <p className="text-sm font-medium text-gray-900">{k.display_name}</p>
                    {k.description && (
                      <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{k.description}</p>
                    )}
                  </td>
                  <td className="py-2.5 px-4"><DomainPill domain={k.domain} /></td>
                  <td className="py-2.5 px-4 text-xs text-gray-500">{k.aggregation_method}</td>
                  <td className="py-2.5 px-4 text-xs text-gray-500">{k.unit ?? "—"}</td>
                  <td className="py-2.5 px-4 max-w-xs">
                    <code className="text-xs text-gray-400 line-clamp-1">{k.formula ?? "—"}</code>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-400">{data?.total ?? 0} terms</span>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-1.5 text-gray-600 hover:bg-gray-100 rounded-md text-sm"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="bg-white rounded-lg border divide-y">
        {isLoading ? (
          <div className="p-8 text-center text-gray-400">Loading glossary…</div>
        ) : terms.length === 0 ? (
          <div className="p-8 text-center text-gray-400">
  ...
  return (
    <div className="flex items-center gap-3 p-4 bg-white border rounded-lg">
      <div className="flex-1">
        <p className="text-sm font-medium text-gray-700">Semantic layer actions</p>
      ...
      <button
        ...
        className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-md text-sm disabled:opacity-50 hover:bg-gray-200"
      >
        {aiMap.isPending ? "Mapping…" : "AI Map Unmapped"}
      </button>
      {(applyPack.isSuccess || aiMap.isSuccess) && (
        <span className="text-xs text-green-600 font-medium">Job queued ✓</span>
      )}
    </div>
  );
}
...
export default function SemanticLayerPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Semantic Layer</h1>
