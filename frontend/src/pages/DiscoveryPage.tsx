...
function DiscoveryPanel({ connectionId }: { connectionId: string }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border p-4 space-y-3">
      ...
      {stage === "error" && (
        <p className="text-sm text-red-600 dark:text-red-400">Error: {status.data?.detail}</p>
      )}
    </div>
  );
}
...
function ColumnRow({ col }: { col: CatalogTableDetail["columns"][0] }) {
  return (
    <tr className="border-b last:border-0">
      <td className="py-2 pr-3 font-mono text-sm text-gray-800 dark:text-gray-100">{col.column_name}</td>
      <td className="py-2 pr-3 text-sm text-gray-500 dark:text-gray-400">{col.data_type}</td>
      ...
      <td className="py-2 pr-3 text-center">
        {col.is_pii_flagged && (
          <span className="text-xs bg-red-100 text-red-700 dark:text-red-300 px-1.5 py-0.5 rounded">PII</span>
        )}
      </td>
      <td className="py-2 text-sm text-gray-400 dark:text-gray-500 max-w-xs truncate">
...
function TableDetailPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-40 flex justify-end">
      <div className="w-full max-w-2xl bg-white dark:bg-gray-800 h-full overflow-y-auto shadow-xl">
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <button onClick={onClose} className="text-gray-400 dark:text-gray-500 hover:text-gray-700 text-xl">
            ×
          </button>
        </div>
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded"
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">Table contains PII</span>
            </label>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-xs text-gray-400 dark:text-gray-500 uppercase border-b">
...
function CatalogRow({
  ...
  return (
    ...
    >
      <td className="py-2.5 pr-3 font-mono text-sm text-blue-700 dark:text-blue-300">{table.schema_name}</td>
      <td className="py-2.5 pr-3 font-mono text-sm font-medium text-gray-900 dark:text-gray-100">
        {table.table_name}
      </td>
      ...
      <td className="py-2.5 pr-3 text-sm text-gray-400 dark:text-gray-500">
...
export default function DiscoveryPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Metadata Catalog</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-gray-400 dark:text-gray-500">Loading catalog…</div>
        ) : tables.length === 0 ? (
